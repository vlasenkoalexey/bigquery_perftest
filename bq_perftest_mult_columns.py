...
def get_dataset_from_bigquery():
    ...
    #         "output_type": (dtypes.string if name.startswith("cat") else dtypes.int64),
    #     }
    ...
    print(
        ...
        % (len(streams), FLAGS.requested_streams)
    )

    dataset = (
        read_session.read_rows(streams[0])
        if FLAGS.requested_streams == 1
        else read_session.parallel_read_rows(
            sloppy=FLAGS.sloppy, num_parallel_calls=len(streams)
        )
    )
